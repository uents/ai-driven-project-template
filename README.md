...
│
├── services/
│    └── user-service/                 # マイクロサービス（機能単位で分離）
│         ├── create-user/             # ユーザー作成（spec.md + src/）
│         └── get-user/                # ユーザー取得（spec.md + src/）
│
├── tests/
│    ├── integration/                  # 統合テスト
│    ├── e2e/                          # E2Eテスト
...
Agentが参照する知識ベース・リファレンス。instructionsが「何をすべきか」なら、skillsは「どうやるか」の詳細知識。
...
繰り返し使う定型プロンプトのテンプレート。作業品質の標準化に使用。
...
| `user-service/{機能名}/spec.md` | 機能仕様書 |
| `user-service/{機能名}/src/` | 機能の実装コード |
| `user-service/{機能名}/src/tests/` | 機能の単体テスト |
