...
│    ├── agents/                       # エージェント定義（{persona}.agent.md）
│    ├── prompts/                      # プロンプトテンプレート（{purpose}.prompt.md）
...
エージェントの役割別ペルソナ定義。「誰として振る舞うか」をエージェントに指示。`{persona}.agent.md` 形式で配置する。

| パス                        | 内容・役割                               |
| --------------------------- | ---------------------------------------- |
| `agents/architect.agent.md` | 設計エージェント：アーキテクチャ設計用   |
| `agents/developer.agent.md` | 実装エージェント：機能設計・コード実装用 |
| `agents/tester.agent.md`    | テストエージェント：テスト設計・実装用   |

#### prompts/（プロンプトテンプレート）

プロンプトのテンプレート。作業品質の標準化に使用。`{purpose}.prompt.md` 形式で配置し、エージェントを適宜呼び出す。

| パス                               | 内容・役割                                   |
| ---------------------------------- | -------------------------------------------- |
| `prompts/feature-design.prompt.md` | 機能設計依頼時のプロンプトテンプレート       |
| `prompts/feature-coding.prompt.md` | コード実装依頼時のプロンプトテンプレート     |
| `prompts/code-review.prompt.md`    | コードレビュー依頼時のプロンプトテンプレート |
| `prompts/bug-fix.prompt.md`        | バグ修正依頼時のプロンプトテンプレート       |

#### skills/（エージェントスキル）

エージェントが特定のタスクを実行するための知識ベース。`{skill-name}/SKILL.md` 形式で配置する。
instructionsが「何をすべきか」なら、skillsは「どうやるか」の詳細知識。

| パス                            | 内容・役割                                    |
| ------------------------------- | --------------------------------------------- |
| `skills/create-adr/SKILL.md`    | Architecture Decision Record の作成方法       |
| `skills/create-readme/SKILL.md` | README.md の構成・記述ルール                  |
...
| `architecture/diagrams/*.pu` | PlantUML形式の構成図（C4モデル）                                 |
...
| `{サービス名}/{機能名}/docs/design.md`     | 機能の設計書                                         |
| `{サービス名}/{機能名}/docs/diagrams/*.pu` | PlantUML形式の設計図（クラス図、シーケンス図等）     |
