...
エージェントの役割別ペルソナ定義。タスクに応じて「誰として振る舞うか」をエージェントに指示する。`{name}.agent.md` 形式で配置する。
...
プロンプトのテンプレート。作業品質の標準化に使用。`{purpose}.prompt.md` 形式で配置し、エージェントに適宜呼び出して使用する。

| パス                            | 内容・役割                                   |
| ------------------------------- | -------------------------------------------- |
| `prompts/design-impl.prompt.md` | 機能設計依頼時のプロンプトテンプレート       |
| `prompts/code-impl.prompt.md`   | コード実装依頼時のプロンプトテンプレート     |
